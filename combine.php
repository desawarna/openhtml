...
//Export Sorted Data into a duplicate table to merge old data set and new data set
	mysql_query("CREATE TABLE replay_combined2 LIKE replay_sessions");
	mysql_query("INSERT INTO replay_combined2 SELECT * FROM replay_sessions");

	echo "<html><head><title>foo</title></head><body>";
	...
	foreach ($pages as $key => $value) {
		...
		$query = sprintf("INSERT INTO replay_combined2 (url, time, session) VALUES ('%s', '%s', '%s')", mysql_real_escape_string($value['url']), mysql_real_escape_string($time), mysql_real_escape_string($value['session']) );
